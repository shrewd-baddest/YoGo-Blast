...
tl.from(
  ...
  const Buy = (product) => {
    ...
    setQuantity(quant);
    const total_pay = pays * quant; // Use quant, not quantity state
    setTotals(total_pay);

    if (pay.current) {
      pay.current.style.display = 'block';
      cartProd.style.filter='blur(10px)';
      cartProd.style.pointerEvents='none';
    }
  }
  ...
  try {
    const res= await axios.post('http://localhost/npm/payment.php', paymentData, { withCredentials: true });
    const response=await fetch('http://localhost/npm/callback.php', {credentials: 'include' });
    const data = await response.json();
    if (data.data.status === 'success') {
      alert(data.data.message);
      navigate('/home');
    } else {
      alert(data.data.message || JSON.stringify(response.data));
    }
  } catch (error) {
    ...
    <div>
      {products && products.length > 0 ? (
        products.map((product) =>(
          <div key={product.products_id} className='cart-products' ref={cartProd} >
...
<button onClick={() => deleteProduct(product)} className='delete' >
  Delete
       ...
       <label>Phone Number:</label>
              <input type="tel" onChange={(e) => setNumber(e.target.value)} value="254" placeholder='254xxxxxxxxx' maxLength='12' />
