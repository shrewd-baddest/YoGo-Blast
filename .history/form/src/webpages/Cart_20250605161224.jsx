...
const Cart = () => {
  ...
  const pay=useRef();
  const cartrod=useRef();
  const [totals,setTotals]=useState(null);
  // const [deletingId, setDeletingId] = useState(null);
  //  console.log(products);
   const fetchCartroducts = async () => {
    try {
...
tl.from(
  ...
  const Buy = (product) => {
    ...
    if (pay.current) {
      ...
      cartrod.style.backdropFilter='blur(8px)';
      cartrod.style.pointerEvents='none';
    }
  }
    ...
    <div>
      {products && products.length > 0 ? (
        products.map((product) =>(
          <div key={product.products_id} className='cart-products' ref={cart-prod} >
